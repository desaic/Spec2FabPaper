...
simulation method for the appearance of translucent materials [59]. 
They
...
and the use of pre-computation, similar to pre-computed light transport approaches [63].
\subsection{Simulation of Deformation Behavior}
In previous work, the problem
of modeling and simulating soft materials and their deformation behavior [22, 60].
...
with our soft tissue modeling technique using scattered data interpolation [60].
...
\begin{verbatim}
Manager manager;
manager.loadMaterialLibrary("material_file");
manager.setGenerator(new HeightFieldGenerator());
manager.setSimulator(new SomeColorSimulator());
manager.setMetric(new SomeColorMetric());
manager.setOptimizer(
  new SimulatedAnealing/GradientDescent());
Objective obj;
Scene scene;
...
To generate BRDF
\begin{verbatim}
manager.setGenerator(new StackingGenerator());
manager.setSimulator(new BRDFSimulator());
manager.setMetric(new SomeImageMetric());
...
Deformation
\begin{verbatim}
scene.addForce(new Force(pos, dir, shapeID));
scene.addConstraint(
  new Constraint(
    FIX_VERTEX, vertexList, shapeID));
Shape goal;
manager.setSimulator(new FEMSimulator());
\end{verbatim}
...
%  	\includegraphics[width=\textwidth]{a}
    ...
    \label{fig:ssPrint}
\end{subfigure}
\caption{Subsurface Scattering. (a)Original object. (b)Printed model.}
\label{fig:SS}
\end{figure}
...
\end{subfigure}
\caption{Matching deformation. (a)photo of a slab being pushed. (b)Printed model being pushed.}
\label{fig:deform}
\end{figure}
