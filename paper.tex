...
Table~\ref{tab:built-in} shows a list of nodes implemented in our framework. With this set of nodes we can reproduce all previous work previously discussed. This validates our assertion, that the space of fabrication  algorithms can be spanned effectively using the Reducer and Tuner networks. Secondly it illustrates how we can leverage component reuse to quickly create such algorithms. Table~\ref{tab:example} lists the components of the framework used to recreate each example.
\begin{table*}
\centering
\begin{tabular}{lc}
\hline 
...
The first version of this algorithm computes  a set of micro-lenses which produce the desired caustic image. The optimization applies simulated annealing in order to permute the location of these micro-lenses in order to achieve a smooth surface. Of note is the fact that this algorithm does not quire a simulation algorithm. Despite this it is still implementable within our framework. 

In the second algorithm , we use subdivision surfaces to represent a smooth caustic caustic surface. This is in contrast to the potentially discontinuous surface in the method above. The original algorithm used B-spline surfaces. We also replace the specialized renderer from the original publication with a generic, fast caustics rendering technique ~\cite{Yuksel:2009}. Our algorithm took 6 hours to finish with $4000$ control points for the model and $600\times 600$ resolution for the caustic image.  A picture of our printed result under desired lighting is shown in Figure~\ref{fig:caustic}.
\begin{figure}
	\centering
...
separate problems. This shows how our framework can facilitates the creation of new fabrication problems by allowing easy reuse of previous work and also providing a ``playground'' in which one can explore different computational approaches.
