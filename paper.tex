...
\section{Results}
We used various Reducer Nodes , simulator and optimization algorithms in our experiments.
A summary of primitive components is listed in Table~\ref{tab:built-in}.
Table~\ref{tab:example} lists for each example, the components involved.
\begin{table*}
\centering
\begin{tabular}{p{4cm}|p{4cm}|p{4cm}}
\hline 
Reducer Node & simulator & optimization\\
\hline 
Geometry Node & optics & \multirow{4}{4cm}{Error diffusion, SPSA, Branch and bound}\\
 InputMesh, Stratum, Column&
 Reflectance and transmittance, caustics\\ \cline{1-2}
Material Node & deformation  \\
ConstantMaterial, 
LayeredMaterial, QuadLayerMaterial, SubdivSurfMaterial & 
...
\begin{table*}
\centering
\begin{tabular}{p{4cm}|p{4cm}|p{4cm}|p{4cm}}  %error metric is not interesting
\hline 
Example & Reducer Nodes & simulator & optimization\\
\hline 
...
We expand the set of printable colors by stacking translucent materials.
We then applied dithering with our expanded colors.
...
We replicated two algorithms 
presented in \cite{Marios:2011} and \cite{Finckh:2010} for computing goal-based caustics.
The first version computes a 2D grid composed of micro-lenses. 
The optimization permutes the location of the micro-lenses using SA to achieve a smooth surface.
...
The tuners are run in sequence until some convergence criteria is hit.
(it's also easy to implement a parallel version).
This algorithm doesn't require any simulation.

In the second algorithm , we used subdivision surfaces to represent the surface for simplicity.
The original algorithm used B-spline surfaces.
We used a generic caustics shader for height field 
instead of a specialized shader for spline surfaces.
Our algorithm took 6 hours to finish with $4000$ control points for the model and $600\times 600$ resolution for the caustic image. 
A picture of our printed result under desired lighting is shown in Figure~\ref{fig:caustic}.
\begin{figure}
	\centering
...
We first replicated \cite{Bickel:2010} to 
print materials with specified elasticity.
We then combined it with subsurface scattering
matching to print material that approximates
both the subsurface scattering property and the deformation behavior.
The result is shown in Figure~\ref{fig:deform}.
The combination is done by simply alternating tuners for the two
separate problems.
